...
def svmkflod(path,outname):
    ...
    for i, (train_index, test_index) in enumerate(rskf.split(X, y)):
        ...
        if (i+1) % n_splits == 0:
            ...
            # 保存最佳模型
            dump(best_model, f'/home/abcdlj/Gam-p/final/models/SVM_{outname}_best_model_{int((i+1)/n_splits)}.joblib')             
            df_best = pd.DataFrame({'y_test': best_y_test, 'y_pred': best_y_pred, 'y_prob': best_y_prob})
            df_best.to_csv(f'/home/abcdlj/Gam-p/final/predict/SVM_{outname}_best_{int((i+1)/n_splits)}.csv', index=False)
            repeat_results = []
            repeat_models = []            
    ...
    df_scores.to_csv('/home/abcdlj/Gam-p/final/predict/SVM_{}_results.csv'.format(outname), index=False)   
