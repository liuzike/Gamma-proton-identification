...
def xgbkflod(path,outname):
    ...
    for i, (train_index, test_index) in enumerate(rskf.split(X, y)):
        ...
        # 创建模型
        # 创建XGBoost分类器
        model = xgb.XGBClassifier(learning_rate=0.1,n_estimators=500,max_depth=8,min_child_weight=0.01,scale_pos_weight=ratio)
        # 训练模型
        ...
        if (i+1) % n_splits == 0:
            ...
            # 保存最佳模型
            dump(best_model, f'/home/abcdlj/Gam-p/final/models/XGB_{outname}_best_model_{int((i+1)/n_splits)}.joblib') 
            df_best = pd.DataFrame({'y_test': best_y_test, 'y_pred': best_y_pred, 'y_prob': best_y_prob})
            df_best.to_csv(f'/home/abcdlj/Gam-p/final/predict/XGB_{outname}_best_{int((i+1)/n_splits)}.csv', index=False)
            repeat_results = []
            repeat_models = []
    ...
    df_scores.to_csv('/home/abcdlj/Gam-p/final/predict/XGB_{}_results.csv'.format(outname), index=False)        
